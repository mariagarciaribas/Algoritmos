...
# Convertir `data$class` a factor (si no lo es ya)
data_pca$class <- as.factor(data_pca$class)

# Crear una nueva columna `class_num` con valores numéricos para cada nivel de `class`
data <- data_pca %>%
  mutate(class_num = as.numeric(factor(class, levels = c("CGC", "CHC", "CFB", "AGH"))))


# Separar las características y la variable objetivo
features <- data[ , !(names(data) %in% c("class", "class_num"))]
